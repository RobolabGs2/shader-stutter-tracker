...
func _add_scenes_from_directory(res: SSTSceneExtractorPrecompilerConfig) -> void:
	...
	open_dir_dialog.add_option("Recursive", [], 1)
	...
	open_dir_dialog.dir_selected.connect(
		func (path: String): 
			res.add_scenes_from_directory(path, open_dir_dialog.get_selected_options()["Recursive"]),
		CONNECT_ONE_SHOT,
		)
...
func _create_buttons(object: Object) -> Control:
	...
	container.add_child(_create_button("Add scenes from directory", func(): _add_scenes_from_directory(object)))
	return container
